...
import {
  IAgentCreate,
  IAgentNameVerification,
} from "../lib/validators/agent.validator";
...
export class AgentController {
  public static readonly primary = async (c: Context) => {
    try {
      ...
      const agentId = await SessionService.getAgentId(
        api_key.userId,
        api_key.id,
      );
      ...
      return c.json(
        api_response({ message: "Agent response", data: agentResponse }),
      );
    } catch (error) {
      ...
      if (
        errorMessageIncludes(error, "Agent URL or provider not found") ||
        errorMessageIncludes(error, "Agent not found")
      ) {
        return c.json(
          api_response({
      ...
      if (
        ...
        errorMessageIncludes(error, "payment")
      ) {
        return c.json(
          api_response({
      ...
      if (
        ...
        errorMessageIncludes(error, "authentication")
      ) {
        return c.json(
          api_response({
      ...
      if (
        errorMessageIncludes(error, "rate limit") ||
        errorMessageIncludes(error, "too many requests")
      ) {
        return c.json(
          api_response({
      ...
      if (
        ...
        errorMessageIncludes(error, "connection")
      ) {
        return c.json(
          api_response({
            message:
  ...
  public static readonly verifyAgent = async (c: Context) => {
    try {
      const { deployedUrl, default_agent_name } =
        (await c.req.json()) as IAgentNameVerification;
      if (!deployedUrl && !default_agent_name) {
        return c.json(
          api_response({
            message: "deployedUrl and default_agent_name are required",
          }),
          400,
        );
      }
      ...
      return c.json(
        api_response({
          message: "Agent URL verified successfully",
          data: true,
        }),
        200,
      );
      ...
      if (error instanceof Error) {
        return c.json(
          api_response({ message: error.message, data: false, is_error: true }),
          500,
        );
      }
    }
  ...
  public static readonly createAgent = async (c: Context) => {
    try {
      ...
      return c.json(
        api_response({ message: "Agent created successfully", data: agent }),
      );
    } catch (error) {
      const errorMessage = getErrorMessage(error);
      const errorStack = getErrorStack(error);
      ...
      // Handle specific error types
      if (
        errorMessageIncludes(error, "duplicate") ||
      ...
      if (
        errorMessageIncludes(error, "embedding") ||
        errorMessageIncludes(error, "generation failed")
      ) {
        return c.json(
          api_response({
            message:
              "Failed to generate agent embedding. Please try again later.",
            is_error: true,
          }),
          503,
        );
      ...
      if (
        errorMessageIncludes(error, "contract") ||
        errorMessageIncludes(error, "blockchain")
      ) {
        return c.json(
          api_response({
            message:
              "Failed to register agent on blockchain. Please try again later.",
            is_error: true,
          }),
          503,
        );
  ...
  public static readonly getAgent = async (c: Context) => {
    try {
      ...
      return c.json(
        api_response({ message: "Agent retrieved successfully", data: agent }),
      );
    } catch (error) {
      const errorMessage = getErrorMessage(error);
      const errorStack = getErrorStack(error);
  ...
  public static readonly updateAgent = async (c: Context) => {
    try {
      ...
      return c.json(
        api_response({ message: "Agent updated successfully", data: agent }),
      );
    } catch (error) {
      const errorMessage = getErrorMessage(error);
      const errorStack = getErrorStack(error);
      ...
      if (errorMessageIncludes(error, "embedding")) {
        return c.json(
          api_response({
            message:
              "Failed to update agent embedding. Please try again later.",
            is_error: true,
          }),
          503,
        );
  ...
  public static readonly deleteAgent = async (c: Context) => {
    try {
      ...
      return c.json(
        api_response({ message: "Agent deleted successfully", data: agent }),
      );
    } catch (error) {
      const errorMessage = getErrorMessage(error);
      const errorStack = getErrorStack(error);
